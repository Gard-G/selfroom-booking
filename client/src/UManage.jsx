...
const UserManagement = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      ...
      const storedStatus = localStorage.getItem('IDstatus'); // Assuming role is stored in localStorage

      // Check if user is an admin
      if (storedStatus !== 'admin') {
        alert('Access denied: Admins only');
        navigate('/'); // Redirect to unauthorized page or login
        return;
      }
  ...
  const handleAddUser = async () => {
    ...
    try {
      ...
      setUsername('');
      setPassword('');
      setIdstatus('user'); // Reset to default
